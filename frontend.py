...
class LoginWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        self.setStyleSheet("""
            QWidget {
                background-color: #F0F2F5;  /* Light elegant background */
                color: #2E2E3E;
            }
            QLabel {
                color: #1C1C1C;
...
class ApplicationsTab(QWidget):
    ...
    def scan_for_updates(self):
        ...
        progress.label.setText("Scanning system for updates...")
        progress.show()
...
class DevicesTab(QWidget):
    ...
    def scan_device(self, device_id):
        try:
            progress = ScanProgressDialog(self)
            progress.label.setText("Scanning remote device...")
            progress.show()
            ...
            if response.status_code == 200:
                scan_data = response.json().get("scan_results", {})
                updates = scan_data.get("available_updates", [])

                if not updates:
                    QMessageBox.information(self, "Scan Result", "No updates available on the remote device.")
                else:
                    # Fill scan results table
                    self.scan_results_table.setRowCount(len(updates))
                    for row, update in enumerate(updates):
                        self.scan_results_table.setItem(row, 0, QTableWidgetItem(update.get("name", "Unknown")))
                        self.scan_results_table.setItem(row, 1, QTableWidgetItem(update.get("Current_version", "Unknown")))
                        self.scan_results_table.setItem(row, 2, QTableWidgetItem(update.get("Available Update", "Unknown")))
            else:
                QMessageBox.warning(self, "Error", "Failed to scan remote device")
...
if __name__ == "__main__":
    ...
    palette.setColor(QPalette.ColorRole.AlternateBase, QColor("#2E2E3E"))
    palette.setColor(QPalette.ColorRole.Text, QColor("#DCDCDC"))
    palette.setColor(QPalette.ColorRole.Button, QColor("#2D2D3C"))
